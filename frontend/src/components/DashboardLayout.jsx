...
const sections = [
  ...
  },
  {
    title: 'KHO HÀNG', roles: ['ROLE_ADMIN', 'ROLE_WAREHOUSE_MANAGER'], items: [
      ...
      { label: 'Phiếu nhập', path: '/stock/in', roles: WH, icon: 'M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5' },
      { label: 'Phiếu xuất', path: '/stock/out', roles: WH, icon: 'M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12M12 16.5V3' },
...
export default function DashboardLayout({ title, children }) {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-purple-200 selection:text-purple-900">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 z-50 h-full ${collapsed ? 'w-[72px]' : 'w-[260px]'} bg-white/80 backdrop-blur-xl border-r border-gray-100 transform transition-all duration-300 lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className={`h-20 flex items-center ${collapsed ? 'justify-center' : 'gap-2 px-6'} border-b border-gray-100 shrink-0 transition-all duration-300`}>
          ...
          {!collapsed && <span className="text-base font-semibold tracking-tight text-gray-900 whitespace-nowrap overflow-hidden">Minh Thang<span className="opacity-30">_</span></span>}
        </div>
        ...
        <nav className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-transparent">
          <div className="px-6 h-20 flex items-center justify-between max-w-[1440px] mx-auto">
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => setCollapsed(!collapsed)} className="hidden lg:flex p-2 -ml-2 rounded-xl hover:bg-gray-50 text-gray-600 transition-colors">

  ...
  return (
    <DashboardLayout>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="p-6">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Thông tin đơn hàng</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <Field label="Ngày giao hàng" type="date" value={deliveryDate} onChange={(e) => setDeliveryDate(e.target.value)} />
            <div className="sm:col-span-2">
              <TextArea label="Địa chỉ giao hàng" value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)} rows={2} />
            </div>
            <div className="sm:col-span-2">
        ...
        <Card className="p-6">
          ...
          <Table headers={['Sản phẩm', 'Số lượng', 'Đơn giá', 'Thành tiền', '']}>
            {items.map((item, idx) => (
              <tr key={item.key}>
                <Td>
                  <select
        ...
        <div className="flex items-center gap-3">
          ...
          <Btn variant="ghost" type="button" onClick={() => navigate('/sales-orders')}>Hủy</Btn>
        </div>
      </form>
    </DashboardLayout>
  )

...
public class AccountantSupplierController {
    ...
    public SetSupplierLinkedMaterialsResult replaceLinkedMaterials(
            @PathVariable UUID id, @RequestBody SetSupplierLinkedMaterialsRequest request) {
        return materialService.replaceLinkedMaterialsForSupplier(id, request.getMaterialIds());
    }

    @PostMapping("/{id}/materials")
    @ResponseStatus(HttpStatus.CREATED)
    public MaterialResponse addLinkedMaterial(
            @PathVariable UUID id, @Valid @RequestBody LinkOneMaterialToSupplierRequest request) {
        return materialService.linkMaterialToSupplier(id, request.getMaterialId());
    }

    @DeleteMapping("/{id}/materials/{materialId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void removeLinkedMaterial(@PathVariable UUID id, @PathVariable UUID materialId) {
